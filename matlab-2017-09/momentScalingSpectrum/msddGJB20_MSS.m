function [DiF] = msddGJB20_MSS(TR, pixelSize, Tau, Fit, Dboundary, interactive3, bigorsmall,addformerhist,genoutput, n)
...
for j=1:numel(mecmec) %NrOfTrs      check all trajectory numbers, if they are in the dataset
    ...
    if trace(1,:)~=zeros(1,rr) % only calculate MSD if 'trace' is not just a zero vector.
        ...
        mol(:,1)=pixelSize*(trace(:,1)-min(trace(:,1)));		%X-co�rdinaten omreken naar �meters en relative nul geven
        mol(:,2)=pixelSize*(trace(:,2)-min(trace(:,2)));		%Idem Y-co�rdinaten
        ...
        for i=1:(newtracelength - 1)                             %MSD punt 1 (X2-X1, ... Xn-Xn-1) tot en met n-1 (Xn-X1) van de trace met lengte n.
            ...
            DISP=(molAA-molBB).^2;							    %squared displacement per co�rdinaat
...
%         end;                % the error in D becomes errorD=MSDoffset/4*Timelag=0.01/4*0.05*6=0.008 �m2/s
...
%                             % gives incorrect fits for low D.
        ...
        if Fit==1
            ...
            %%% JR %%% TmsdFit=[0;MSD(1:fitlength+0.5,1)];
            TmsdFit=[0;MSD(1:fitlength,1)];
        ...
        elseif Fit==2
            ...
            [X,resnorm,residual,eflag,out,lambda,jaco] = lsqnonlin(@FitMeanMSDtalphaGJB18,x0,[0,0,0],[40,0.05,5],[], MSD(:,3), MSD(:,1), MSD(:,2), fitlength);
...
% % stdfit=sqrt(resultfitD.sig2);    % standard deviation, sigma of fit [�m^2/s]
% %nn=max(hist(N3));
...
% hist(DiF(:,1),15);
% plot(XX,y);
% text(0.8*min(xout3),0.8*max(N3), ['Sigma = ', num2str(sig), ' �m\^2 / sec'] ,'HorizontalAlignment','left','FontSize',12, 'color',[0 0 0], 'FontWeight', 'bold');  
% hold off
