function analyzeSpotIntensities(varargin)
    ...
    % compute bgX and bgY
    %	cosd (cosine in degrees) from 0 to 315 in steps of 45
    %		gen.values=start:step:end
    % equivalent to cos( [ 0*pi, 1/2*pi, pi, 3/2*pi] ) or
    % cos( [0:3]*pi/2 )
    bgX = k0R*cosd(0:45:315);		% magic numbers here (uh, oh?)
    bgY = k0R*sind(0:45:315);		% ditto. Check why. %%% JR %%%
	...
	%%% JR %%% shouldn't it be k0Percentile???
