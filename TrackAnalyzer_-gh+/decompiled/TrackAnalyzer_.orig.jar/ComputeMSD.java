   ...
   public void Compute(List<Integer> nOfTracks, ResultsTable rtSpots) {
      ...
      for(i = 0; i < imported2SpotX.size(); ++i) {
         ...
         for(dt = 0; dt < tauMSD.length; ++dt) {
            ...
            for(z = tauMSD[dt]; z < ((List)imported2SpotX.get(i)).size(); ++z) {
               msdT += Math.pow((Double)((List)imported2SpotX.get(i)).get(z - tauMSD[dt]) - (Double)((List)imported2SpotX.get(i)).get(z), 2.0D) + Math.pow((Double)((List)imported2SpotY.get(i)).get(z - tauMSD[dt]) - (Double)((List)imported2SpotY.get(i)).get(z), 2.0D);
               ++NMSD;
            }
         ...
         int[] tauD1N = new int[this.nOfPoints];

         for(z = 0; z < this.nOfPoints - 1 + 1; ++z) {
            tauD1N[z] = 1 + z;
         }

         SimpleRegression regD1N = new SimpleRegression(true);
         double msd1N = -1.0D;
         double msdhelp1N = 0.0D;

         for(int dt1N = 0; dt1N < tauD1N.length; ++dt1N) {
            double N1N = 0.0D;
            msd1N = 0.0D;

            for(int j = tauD1N[dt1N]; j < ((List)imported2SpotX.get(i)).size(); ++j) {
               msd1N += Math.pow((Double)((List)imported2SpotX.get(i)).get(j - tau[dt1N]) - (Double)((List)imported2SpotX.get(i)).get(j), 2.0D) + Math.pow((Double)((List)imported2SpotY.get(i)).get(j - tau[dt1N]) - (Double)((List)imported2SpotY.get(i)).get(j), 2.0D);
               ++N1N;
            }

            msd1N /= N1N;
            regD1N.addData((double)tauD1N[dt1N] * frameInterval, msd1N);
         }

         double slopeDiff1N = regD1N.getSlope() / (double)this.nOfPoints;
         ...
         d1NValues.add(Math.abs(slopeDiff1N));
