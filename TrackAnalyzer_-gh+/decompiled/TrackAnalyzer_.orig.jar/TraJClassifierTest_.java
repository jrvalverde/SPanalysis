   ...
   public void run(String arg) {
      ...
      if (!arg.contains("DEBUG")) {
         ...
         while(var7.hasNext()) {
            ...
            while(var16.hasNext()) {
               dcEstim = (Spot)var16.next();
               trackID.add((float)id);
               framesByTrack.add(Float.valueOf(dcEstim.getFeature("FRAME").toString()));
               prois.add(Float.valueOf(dcEstim.getFeature("FRAME").toString()));
               spots.add(dcEstim);
            }
            ...
            for(subPosCount = 0; subPosCount < indexes.size(); ++subPosCount) {
               imported1Spot.add((Spot)spots.get((Integer)indexes.get(subPosCount)));
            }
      ...
      Iterator var59 = this.tracksToClassify.iterator();

      while(var59.hasNext()) {
         track = (TrajectoryModified)var59.next();
      ...
      if (this.parentTrajectories.isEmpty() == Boolean.FALSE) {
         ...
         if (visualize) {
            ...
            while(true) {
               ...
               Iterator var70 = prois.iterator();

               while(var70.hasNext()) {
                  Roi r = (Roi)var70.next();
                  ov.add(r);
               }
         ...
         for(y = 0; y < this.classifiedSegments.size(); ++y) {
            ...
            this.type = t.getType();
            ResultsTable rtT = (ResultsTable)rtables.get(this.type);
            if (rtT == null) {
               SPTBatch_.taskOutput.append("Type: " + this.type);
            ...
            if (this.type != null) {
               String var25;
               switch((var25 = this.type).hashCode()) {
               case -1768066899:
                  if (var25.equals("DIRECTED/ACTIVE")) {
               ...
               case -568951419:
                  ...
                  break;
               case 160958711:
                  if (var25.equals("NORM. DIFFUSION")) {
            ...
            if (t.getType() != null && !t.getType().equals("NONE")) {
         ...
         while(rtIt.hasNext()) {
            ...
            if (rt.equals("DIRECTED/ACTIVE") == Boolean.TRUE) {
               ((ResultsTable)rtables.get(rt)).save(SPTBatch_.directDiff + File.separator + "DIRECTED_ACTIVE" + "_trajectories_" + SPTBatch_.imps.getShortTitle() + ".xls");
            } else {
               ((ResultsTable)rtables.get(rt)).save(SPTBatch_.directDiff + File.separator + rt + "_trajectories_" + SPTBatch_.imps.getShortTitle() + ".xls");
            }
         }
      }
